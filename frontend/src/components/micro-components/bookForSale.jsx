...
export default function Book ( {book} ) {
  ...
  const handleClick = () => {
    ...
    <div onClick={handleClick} className="icon-box" data-aos="fade-up" data-aos-delay="100">
      <img src= {book.image && book.image.length > 0? book.image : urlImage} className="books-item-image" alt=""></img>
      <div className="books-description">
        <h4 className="title">{book.title}</h4>
        <h3><span>US$</span> {priceIntegerPart} <span> {priceDecimalPart} </span></h3>
        <p> {book.description && book.description.length>50?book.description.substring(0,50)+"...":book.description} </p>
